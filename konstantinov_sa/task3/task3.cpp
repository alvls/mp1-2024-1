...
//¬ основе словар€ св€з€анный список
class Dict {
private:
	Wpair* start;
	Wpair* end;
...
public:
	...
	//ƒобавление в начало, допускает дубликаты.
	...
	//ѕотенциально более медленное добавление в конец, так как иногда потребуетс€ найти конец. »спользуетс€ дл€ загрузки из файла
	...
	//дополнить ещЄ одним вариантом перевода (кажда€ нова€ строка - новый вариант)
	...
	//загружает (добавл€ет) в словарь содержимое из файла, не очищает
...
int main() {
	...
	d.add("nuclear", "€дрЄный");

	d.print();
	cout << "»справим nuclear\n";
	d.replace("nuclear", "€дерный");
	cout << "»щем\n";
	string f = "nuclear";
	cout << f << " - " << d.get(f)<<endl;
	cout << "—лов всего: " << d.count() << endl<<"удалим impostor и сохраним. «агрузим другой словарь\n";
