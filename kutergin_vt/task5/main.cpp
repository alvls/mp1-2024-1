...
int main(int argc, const char * argv[]) {
    try {
        Processing_Center* PC = new Processing_Center();
        Credit CRDT(0, 0.0, 0.0, PC);
        ...
        Credit CR1(1, 100000.0, 12.0, PC);
        Credit CR2(1, 450000.0, 15.0, PC);
        Credit CR3(2, 750000.0, 15.0, PC);
        Credit CR4(3, 800000.0, 12.0, PC);
        Credit CR5(3, 1000000.0, 18.0, PC);
        Credit CR6(5, 1500000.0, 20.0, PC);
        Credit CR7(5, 2000000.0, 22.0, PC);
        Credit CR8(15, 3000000.0, 24.0, PC);
        ...
        string password;
        cin >> password;
        if (!CRDT.authorize(acc_num, password))
            throw invalid_argument("Authorization error");
        
        CRDT.showINFO(acc_num);
        
        cout << "Select a credit from the Available credits menu (enter index):" << endl;
        int choice;
        cin >> choice;
        if (!CRDT.checkCreditExistence(acc_num) && CRDT.checkCreditAvailability(acc_num, choice)) // если у клиента нет кредитов и выбранный кредит пользователю доступен
        {
            CRDT.getCredit(acc_num, choice);
            cout << "You have a credit " << choice << " approved" << endl;
        }
        else
            throw runtime_error("You can't get credit");
        CRDT.showCreditState(acc_num);
        cout << "What part of the credit would you like to pay? " << endl;
        cout << "Enter the payment amount: " << endl;
        double payment;
        cin >> payment;
        CRDT.payInstallment(acc_num, payment);
        CRDT.showCreditState(acc_num);
        CRDT.payOffCredit(acc_num);
        CRDT.showCreditState(acc_num);
